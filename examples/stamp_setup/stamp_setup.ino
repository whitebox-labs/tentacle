...
byte I2C_call(){  					//function to parse and call I2C commands. returns byte: 1 = success, 2 = command failed, 254 = command pending, 255: no data
  ...
  // TODO: check if 2000ms is necessairy (or if 1300ms is enough - see datasheets)
  if(String(cmd) == F("c") || String(cmd) == F("C") || String(cmd) == F("r") || String(cmd) == F("R")) {              //if a command has been sent to calibrate or take a reading we wait 2000ms so that the circuit has time to take the reading. 
    delay(2000);
  } 
  else {
